...
pub async fn update_machines_stats(
    ...
    for (machine, leader_pid) in current_machines.into_iter() {
        ...
        if config.networkd.enabled {
            ...
            join_set.spawn(async move {
                if config_clone.varlink.enabled
                    && config_clone.machines.varlink
                    && config_clone.networkd.varlink
                {
        ...
        let manager_describe =
            (config.varlink.enabled && config.machines.varlink && config.system_state.varlink)
                .then(|| {
        ...
        if config.units.enabled {
            if config.varlink.enabled && config.machines.varlink && config.units.varlink {
                let config_clone = Arc::clone(&config);
                let sdc_clone = sdc.clone();
                let stats_clone = locked_machine_stats.clone();
                let machine_name = machine.clone();
